...
def mask_visualization(image, masks, output_path,img_name, add_bbox):
    ...
    ax.imshow(img)    
    plt.savefig(output_path+img_name.split('.')[0]+f'_candidates_bbox_{add_bbox}.png', bbox_inches='tight', dpi=600, pad_inches=0)
    plt.close()

def final_visualization(img, bboxes, output_path,image_name, add_bbox):
    ...
    for bbox in bboxes:
        ...
        plt.axis('off')
    plt.savefig(output_path+image_name.split('.')[0]+f'_bbox_{add_bbox}.png', bbox_inches='tight', dpi=600, pad_inches=0)
    plt.close()
...
def main():
    ...
    for image_name in tqdm(image_names):
        ...
        # --------------------
        mask_visualization(img, filtered_masks, output_path, image_name, add_bbox)
        candidates_img = Image.open(output_path+image_name.split('.')[0]+f'_candidates_bbox_{add_bbox}.png').convert('RGB')
        
        # ---------------
        # annotate
        # ---------------
        inputs = processor(prompt, intermediate_img, return_tensors="pt").to("cuda")
        ...
        try:
            ...
            masks = [filtered_masks[l]['segmentation'] for l in labels]
        except:
            labels = [-1]
            bboxes = [[0,0,0,0]]
            masks = np.zeros(image_size)
        np.save(output_path+image_name.split('.')[0]+f'_labels_bbox_{add_bbox}.npy', labels)
        np.save(output_path+image_name.split('.')[0]+f'_bbox_bbox_{add_bbox}.npy', bboxes)
        np.save(output_path+image_name.split('.')[0]+f'_mask_bbox_{add_bbox}.npy', masks)
                
        # --------------------
        # visualize bounding box
        # --------------------
        final_visualization(img, bboxes, output_path, image_name, add_bbox)       
