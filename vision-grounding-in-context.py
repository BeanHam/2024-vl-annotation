...
def mask_visualization(image, masks, output_path,save_name, add_bbox):
    ...
    ax.imshow(img)    
    plt.savefig(output_path+save_name+f'_candidates_bbox_{add_bbox}.png', bbox_inches='tight', dpi=600, pad_inches=0)
    plt.close()
...
def final_visualization(img, bboxes, output_path,save_name, add_bbox):
    ...
    for bbox in bboxes:
        ...
        plt.axis('off')
    plt.savefig(output_path+save_name+f'_bbox_{add_bbox}.png', bbox_inches='tight', dpi=600, pad_inches=0)
    plt.close()
...
def main():
    ...
    for image_name in tqdm(image_names):
        ...
        candidates_img = Image.open(output_path+save_name+f'_candidates_bbox_{add_bbox}.png').convert('RGB')
        ...
        np.save(output_path+save_name+f'_labels_bbox_{add_bbox}.npy', labels)
        np.save(output_path+save_name+f'_bbox_bbox_{add_bbox}.npy', bboxes)        
        np.save(output_path+save_name+f'_masking_bbox_{add_bbox}.npy', masking) 
    ...
    files = [file for file in files if f'masking_bbox_{bbox}.npy' in file]
    miou = []
    for file in files:
